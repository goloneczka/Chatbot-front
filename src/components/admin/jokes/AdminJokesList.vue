    ...
    <div class="admin-jokes-list">
        ...
        <div class="container mt-5">
            <h3 class="text-left title">
                <div v-if="showCategories && categories.length > 0">
                    {{ $t('adminJokes.categoriesTitle') }} - {{ $t('adminJokes.addCategoryHeaderName') }}
                    <button class="ml-4 btn btn-outline-secondary" v-on:click="addCategoryShowModal">{{
                        $t('adminJokes.emptyListOfCategoryButton') }}
                    </button>
                    <button class="ml-4 btn btn-outline-secondary" v-on:click="goToUnconfirmedJokes">
                        {{$t('adminJokes.unconfirmedJokes')}}
                    </button>
                </div>
                <div v-if="showJokes && jokes.length > 0">
                    {{ $t('adminJokes.jokesTitle') }} {{ $t('adminJokes.addJokeHeaderName') }}
                    <button class="ml-4 btn btn-outline-secondary" v-on:click="addJokeShowModal">{{
                        $t('adminJokes.emptyListOfJokeButton') }}
                    </button>
                </div>
            </h3>
                ...
                <div v-if="showCategories" class="col-12">
                    <div v-if="categories.length == 0">
                        <div class="text-center mt-4">
                            <h4>{{ $t('adminJokes.emptyListOfCategoryTitle') }}
                                <button class="btn btn-outline-secondary" v-on:click="addCategoryShowModal">{{
                                    $t('adminJokes.emptyListOfCategoryButton') }}
                                </button>
                            </h4>
                        </div>
                    ...
                    <div v-else>
                        <category-item v-for="(category) in categories" :category="category" :key="category.name"
                                       v-on:showJokesForCategory="showJokesForCategory($event)"
                                       v-on:removeCategory="removeCategoryShowAlert($event)"
                                       v-on:editCategory="editCategoryModal($event)"/>
                    </div>
                </div>
                <div v-else-if="showJokes" class="col-12">
                    <div v-if="jokes.length == 0">
                        <div class="text-center mt-4">
                            <h4>{{ $t('adminJokes.emptyListOfJokeTitle') }}
                                <button class="btn btn-outline-secondary" v-on:click="addJokeShowModal">{{
                                    $t('adminJokes.emptyListOfJokeButton') }}
                                </button>
                            </h4>
                        </div>
                    ...
                    <div v-else>
                        <joke-item v-for="(joke) in jokes" :joke="joke" :is-confirmed="true" :key="joke.id"
                                   v-on:removeJoke="removeJoke($event)" v-on:editJoke="editJokeModal($event)"/>
                    </div>
                </div>


                <div v-else-if="showUnconfirmedJokes" class="col-12">
                    <div v-if="jokes.length == 0">
                        <div class="text-center mt-4">
                            <h4>{{ $t('adminJokes.emptyListOfJokeTitle') }}
                                <button class="btn btn-outline-secondary" v-on:click="addJokeShowModal">{{
                    ...
                    <div v-else>
                        <joke-item v-for="(joke) in jokes" :joke="joke" :is-confirmed="false" :key="joke.id"
                                   v-on:confirmJoke="confirmJoke($event)" v-on:removeJoke="removeJoke($event)"
                                   v-on:editJoke="editUnconfirmedJokeModal($event)"/>
                    </div>
                </div>
        ...
        <b-modal id="edit-category-modal" :title="$t('adminJokes.modalEditCategoryTitle')" hide-footer>
            ...
            <form @submit="editCategorySubmit" @submit.prevent>
                ...
                <div class="text-right">
                    <button type="submit" class="btn btn-primary" :disabled="checkEditCategory">{{
                        $t('adminJokes.modalEditCategorySubmit') }}
                    </button>
                </div>
            </form>
        </b-modal>
        <b-modal id="remove-category-alert-modal" :title="$t('adminJokes.modalRemoveCategoryTitle')" hide-footer>
            ...
            <div class="text-center">
                ...
                <button class="btn btn-danger btn-sm" v-on:click="removeCategorySubmit">{{
                    $t('adminJokes.modalRemoveCategoryButton') }}
                </button>
            </div>
        </b-modal>
            ...
            <form @submit="addNewJokeSubmit" @submit.prevent>
                ...
                <div class="text-right">
                    <button type="submit" class="btn btn-primary" :disabled="checkAddJoke">{{
                        $t('adminJokes.modalAddJokeSubmit') }}
                    </button>
                </div>
            </form>
        </b-modal>
            ...
            <form @submit="editJokeSubmit" @submit.prevent>
                ...
                <div class="text-right">
                    <button type="submit" class="btn btn-primary" :disabled="checkEditJoke">{{
                        $t('adminJokes.modalEditJokeButton') }}
                    </button>
                </div>
            </form>
        </b-modal>
        ...
        <b-modal id="edit-unconfirmed-joke-modal" :title="$t('adminJokes.modalEditJokeTitle')" hide-footer>
            <ul v-if="editJoke.errors.length" class="alert alert-danger" role="alert">
                <li v-for="(error) in editJoke.errors" :key="error">{{error}}</li>
            </ul>

            <form @submit="editUnconfirmedJokeSubmit" @submit.prevent>
                <div class="form-group">
                    <label for="inputJoke">{{ $t('adminJokes.modalEditJoke') }}</label>
                    <input type="text" class="form-control" v-model="editJoke.newJoke">
                </div>
                <div>
                    <label>{{ $t('adminJokes.modalEditJokeCategory') }}</label>
                    <b-form-select :options="categoriesOptions" v-model="editJoke.newCategory"></b-form-select>
                </div>
                <div class="text-right">
                    <button type="submit" class="btn btn-primary" :disabled="(checkEditJoke && checkEditJokeCategory)">
                        {{
                        $t('adminJokes.modalEditJokeButton') }}
                    </button>
                </div>
...
<script>
    ...
    export default {
                ...
                .then(
                    response => {
                        ...
                        if (response.errors) {
                            ...
                            response.forEach(o => {
                                this.categoriesOptions.push({value: o.category, text: o.category});
                            });
        ...
        methods: {
            ...
            editUnconfirmedJokeModal(joke) {
                this.editJoke.id = joke.id;
                this.editJoke.newJoke = joke.joke;
                this.editJoke.oldJoke = joke.joke;
                this.editJoke.oldCategory = joke.category;
                this.editJoke.newCategory = joke.category;
                this.$bvModal.show('edit-unconfirmed-joke-modal')
            },
            editJokeSubmit() {
                    ...
                    service.modifyJoke(
                        {
                            id: this.editJoke.id,
                            joke: this.editJoke.newJoke,
                            category: this.editJoke.newCategory
                        }
                    ).then(response => {
                        if (response.errors)
                            this.editJoke.errors.push(this.$t('adminJokes.errorEditResponseJoke'));
                        else {
                            this.$set(this.jokes, this.jokes.findIndex(o => {
                                return o.id === response.id;
                            }), response);

                            this.$bvModal.hide('edit-unconfirmed-joke-modal');
                        }
            ...
            goToUnconfirmedJokes() {

                this.showLoading = true;
                this.error = '';
                this.showCategories = false;

                service.getUnconfirmedJokes()
                    .then(
                        response => {
                            this.showLoading = false;
                            ...
                            if (response.errors) {
                                ...
                                this.jokes = response;
                                this.showUnconfirmedJokes = true
                            }
            ...
            confirmJoke(joke) {
                ...
                service.confirmJoke(joke.id)
                    .then(response => {
                        if (response.errors)
                            this.error = this.$t('adminJokes.errorConfirmJoke') + response.errors[0];
                        else {
                            this.jokes.splice(this.jokes.findIndex(o => {
                                return o.id === response.id;
                            }), 1);
                        }
        ...
        computed: {
            ...
            checkEditJokeCategory() {
                return this.editJoke.newCategory === this.editJoke.oldCategory;
            }
        }
    }
</script>
